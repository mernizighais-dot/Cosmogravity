# ------------------------------------------------------------------
# verification_facteurLCDM.mpl - Scale factor a(t): past AND future
#
# Loops over a from ~0 to a_max (a=1 = today, a>1 = future).
# Output: one CSV with columns t_an, t_Gyr, a
#
# USAGE (temp-file via runLCDM.py, change MAPLE_SCRIPT):
#   a_max := 5.0:  N_pts := 300:
#   (other LCDM parameters as usual)
#   read "C:/Users/theoc/Desktop/cours/mapple/codefullmaple/verification_facteurLCDM.mpl":
# ------------------------------------------------------------------
...
output_prefix := cat("H", H0, "_m", Omega_m0, "_l", Omega_lambda0,
...
fd := fopen(cat(output_prefix, "LCDM_facteur_a.csv"), WRITE, TEXT):
fprintf(fd, "t_an,t_Gyr,a\n"):

for idx from 0 to N_pts do
    a_val := evalf(da_step*idx + da_step*0.001):   # avoid exact 0
